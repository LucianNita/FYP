...
p = DSProblem(4); #10+1*no. of colloc pts
...
obj(x) = max(abs(3048-gu(x)[1]),abs(3048-gl(x)[1]))+(gu(x)[2])^2+(gl(x)[2])^2;#max(abs(3048-gl(x)[1]),abs(3048-gu(x)[1]));

SetInitialPoint(p, [11.33,11.39,28.7,28.8]);
SetObjective(p,obj);
...
function kh(t,x)
    return (x[1]-x[2])/(hu(t,x)-hl(t,x));
end

function k0(t,x)
    return (x[1]-kh(t,x)*hu(t,x));
end
...
plot!(t,khp,seriestype = :scatter, title = "My Scatter Plot");
